"""
ui.py — UI primitives for NeuroSim.
...
Provides:
    ...
    - Fonts         Lazily-initialized font set
    - Button        Clickable rectangular button widget
    - TextInput     Single-line text input with blinking caret
"""
...
# ---------------------------------------------------------------------------
# Color palette  (dark theme inspired by EEG monitoring software)
# ---------------------------------------------------------------------------
class Colors:
...
# ---------------------------------------------------------------------------
# Fonts -- lazily initialized after pygame.init()
# ---------------------------------------------------------------------------
class Fonts:
    title = None
    heading = None
...
class Button:
    ...
    def clicked(self, event: pygame.event.Event) -> bool:
        """Return True if this button was clicked by a mouse-up event."""
        if self.disabled:
            return False
...
class TextInput:
    """
    A single-line text input field with a blinking caret.

    Always active (no click-to-focus needed) -- the parent screen owns
    only one input at a time so this keeps the UX simple.
    """
    ...
    def handle_event(self, event: pygame.event.Event) -> bool:
        """Process a keyboard event. Returns True if Enter was pressed."""
        ...
        # Try the easy path first: event.unicode usually gives us the
        # right character (handles Shift, layouts, etc).
        char = event.unicode
        if char and char.isprintable() and len(self.text) < self.max_length:
            self.text += char
            return False

        # Fallback: some environments don't populate event.unicode for
        # plain letters (e.g. certain IME states on Windows/Mac).
        # Map key codes directly to letters; check Shift for caps.
        if pygame.K_a <= event.key <= pygame.K_z:
            letter = chr(ord('a') + (event.key - pygame.K_a))
            # Prefer the event's own modifier flags; fall back to the
            # global keyboard state if those aren't set (older pygame).
        ...
        # Number keys (top row) -- also fall back here just in case
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        # Blinking caret (only if there's actual user text)
