"""
plot_widgets.py — Custom pygame-based plotting for NeuroSim.

This module is imported by gui_game.py — do NOT run it directly.
Run `python main.py` instead.

Provides two drawing functions:
    - draw_waveform_panel  Multi-channel EEG time-domain plot, with
                           optional right-to-left scrolling animation
                           (mimics a clinical EEG monitor).
    - draw_spectrum_panel  Log-scale power-spectral-density plot,
                           with optional frequency-band highlighting.

We draw directly to pygame surfaces instead of using matplotlib so the
visualization is native to the game window (no extra figure windows).
"""
...
def draw_waveform_panel(surface: pygame.Surface, rect: pygame.Rect,
    ...
    """
    Draw a multi-channel EEG waveform panel inside `rect`.

    Parameters
    ----------
    signal : np.ndarray, shape (n_channels, n_samples)
    scroll_offset : int
        Starting sample index. When > 0, the visible window slides into
        the signal, producing a right-to-left scrolling effect like a
        real EEG monitor. The signal is wrapped (modulo n_samples) so
        it loops seamlessly.
    window_samples : Optional[int]
        How many samples are displayed at once. Defaults to all samples.
    animated : bool
        If True, draw a faint "leading edge" marker on the right side
        (mimics the sweeping cursor of a clinical EEG display).
    """
    # Background panel
...
def draw_spectrum_panel(surface: pygame.Surface, rect: pygame.Rect,
    ...
    """
    Draw the power spectrum (log-scale) inside `rect`.

    If reveal_state is given, the corresponding frequency band is shaded.
    """
    pygame.draw.rect(surface, Colors.PANEL, rect, border_radius=6)
